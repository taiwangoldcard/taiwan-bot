...
class UniversalSentenceEncoderLite:
    ...
    def __init__(self):
        ...
        with tf.Session() as sess:
            ...
            # spm_path now contains a path to the SentencePiece model stored inside the TF-Hub module
    ...
    def extract_embeddings(self, sentences, batch_size=10):
        ...
        for i in range(0, len(sentences), batch_size):
            embedding_batches.append(
                self._get_embeddings(sentences[i:i+batch_size]))
    ...
    def get_similarity_scores(self, sentence_embedding: np.ndarray, embeddings_list: np.ndarray) -> np.ndarray:
        # compute dot product with each question in the database, returns shape (n_questions, 1)
        return embeddings_list @ sentence_embedding.T
